...
class Assistant(object):
    ...
    def run_data_assistant(
        ...
        Based on the data, it uses GX Onboarding Data Assistant to generate expectations
    ...
    def _return_expectations(
        ...
        Args:
            ...
            log_expectations: whether to log expectations to the console or not.
                Can be useful for scenarios in which display won't work
                (e.g. local tests).
        """
        expectations = []
        ...
        pd_df = pd.DataFrame(expectations)
        expectations_df = ExecEnv.SESSION.createDataFrame(pd_df)
    ...
    def _return_profiling(
        ...
        if profiling_path:
            ...
            FileStorageFunctions.write_payload(profiling_path, url, profile)
